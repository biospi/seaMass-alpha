...
# try to find at /opt/intel/compiler
...
if ( WIN32 )
  if(NOT DEFINED ENV{IntelOpenMPROOT_PATH})
    set(IntelOpenMPROOT_PATH "C:/Program Files (x86)/IntelSWTools/compilers_and_libraries/windows" CACHE PATH "Where the IntelOpenMP are stored")
  endif(NOT DEFINED ENV{IntelOpenMPROOT_PATH}) 
else ( WIN32 )
    set(IntelOpenMPROOT_PATH "/opt/intel" CACHE PATH "Where the IntelOpenMP are stored")
endif ( WIN32 )

if (EXISTS ${IntelOpenMPROOT_PATH}/compiler)
    SET(IntelOpenMP_FOUND TRUE)
    message("IntelOpenMP is found at ${IntelOpenMPROOT_PATH}/compiler")
    IF(CMAKE_SIZEOF_VOID_P EQUAL 8)
        set( USE_IntelOpenMP_64BIT On )
    ELSE(CMAKE_SIZEOF_VOID_P EQUAL 8)
        set( USE_IntelOpenMP_64BIT Off )
    ENDIF(CMAKE_SIZEOF_VOID_P EQUAL 8)
else (EXISTS ${IntelOpenMPROOT_PATH}/compiler)
    SET(IntelOpenMP_FOUND FALSE)
    message("IntelOpenMP is NOT found ... ")
endif (EXISTS ${IntelOpenMPROOT_PATH}/compiler)

if (IntelOpenMP_FOUND)
    ADD_DEFINITIONS(-DUSE_IntelOpenMP)
    if ( USE_IntelOpenMP_64BIT )
        set(IntelOpenMP_LIB_DIR "${IntelOpenMPROOT_PATH}/compiler/lib/intel64")
        set(IntelOpenMP_COMPILER_LIB_DIR "${IntelOpenMPROOT_PATH}/compiler/lib/intel64")
        set(IntelOpenMP_COMPILER_LIB_DIR ${IntelOpenMP_COMPILER_LIB_DIR} "${IntelOpenMPROOT_PATH}/lib/intel64")
        if (WIN32)
            set(IntelOpenMP_LIBRARIES ${IntelOpenMP_LIBRARIES} ${IntelOpenMP_LIB_DIR}/libiomp5md.lib)
        else (WIN32)
            set(IntelOpenMP_LIBRARIES ${IntelOpenMP_LIBRARIES} ${IntelOpenMP_LIB_DIR}/libcompiler_intel_ilp64.so)
        endif (WIN32)
    else ( USE_IntelOpenMP_64BIT )
        set(IntelOpenMP_LIB_DIR "${IntelOpenMPROOT_PATH}/compiler/lib/ia32")
        set(IntelOpenMP_COMPILER_LIB_DIR "${IntelOpenMPROOT_PATH}/compiler/lib/ia32")
        set(IntelOpenMP_COMPILER_LIB_DIR ${IntelOpenMP_COMPILER_LIB_DIR} "${IntelOpenMPROOT_PATH}/lib/ia32")
        if ( WIN32 )
            set(IntelOpenMP_LIBRARIES ${IntelOpenMP_LIBRARIES} ${IntelOpenMP_LIB_DIR}/libiomp5md.lib)
        else ( WIN32 )
            set(IntelOpenMP_LIBRARIES ${IntelOpenMP_LIBRARIES} ${IntelOpenMP_LIB_DIR}/libcompiler_intel.so)
        endif ( WIN32 )
...
IF (IntelOpenMP_FOUND)
    IF (NOT IntelOpenMP_FIND_QUIETLY)
        ...
        MESSAGE(STATUS "IntelOpenMP_COMPILER_LIB_DIR: ${IntelOpenMP_COMPILER_LIB_DIR}")
