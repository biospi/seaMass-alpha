...
MatrixSparse::MatrixSparse() : m_(0), n_(0), nnz_(0), mat_(0), isCsrIndexesOwned_(false), isCsrValuesOwned_(false), parent_(0), isSorted_(true)
{
}
...
void MatrixSparse::swap(MatrixSparse& a)
{
    ...
    if(parent_)
    {
        for (ii k = 0; k < parent_->children_.size(); k++)
        {
            if (parent_->children_[k] == this)
            {
                parent_->children_[k] = &a;
                break;
            }
        }
    }
    *this = a;
    ...
    if (a.parent_)
    {
        for (ii k = 0; k < a.parent_->children_.size(); k++)
        {
            if (a.parent_->children_[k] == &a)
            {
                a.parent_->children_[k] = this;
                break;
            }
        }
    }
    ...
    // set this to 0 so we don't free!
    t.nnz_ = 0;
}
...
void MatrixSparse::free()
{
    if (nnz_)
    {
        if (isCsrIndexesOwned_)
        {
            mkl_free(ijs_);
            mkl_free(js_);

            isCsrIndexesOwned_ = false;
        }

        if (isCsrValuesOwned_)
        {
            mkl_free(vs_);

            isCsrValuesOwned_ = false;
        }
...
void MatrixSparse::initWithSharedIndexes(MatrixSparse &x, const MatrixSparse &a)
{
    if (getDebugLevel() % 10 >= 4)
    {
        ostringstream oss;
        oss << getTimeStamp() << "       initWithSharedIndexes(A" << a << ") := ...";
        info(oss.str());
    }
    ...
    if (createCsrWithSharedIndexes(x))
    {
        x.sort();
        a.sort();
        ...
        {
            ii a_nz = a.ijs_[i];
            for (ii nz = ijs_[i]; nz < ijs_[i + 1]; nz++)
            {
                for (; a_nz < a.ijs_[i + 1]; a_nz++)
                {
                    if (x.js_[nz] == a.js_[a_nz])
                    {
                        vs_[nz] = a.vs_[a_nz];
                        break;
                    }
...
ii MatrixSparse::pruneCells(fp threshold)
{
    if (getDebugLevel() % 10 >= 4)
    {
        ostringstream oss;
        oss << getTimeStamp() << "       pruneCells(X" << *this << ", " << threshold << ") := ";
        oss.unsetf(ios::floatfield);
        oss << setprecision(8) << threshold << ") := ...";
        info(oss.str());
    }

    // for this matrix find nnzCells and per-row nnzs after thresholding
    ii nnzCells = 0;
    vector<ii> nnzs;
    if (nnz_ > 0)
    {
    ...
    MatrixSparse a;
    if (a.createCsr(m_, n_, nnzCells))
    {
        vector<MatrixSparse> shared(children_.size());
        for (ii k = 0; k < ii(shared.size()); k++)
            shared[k].createCsrWithSharedIndexes(a);

        a.ijs_[0] = 0;
        for (ii i = 0; i < a.m_; i++)
            a.ijs_[i + 1] = a.ijs_[i] + nnzs[i];
        ...
        for (ii i = 0; i < a.m_; i++)
        {
            ...
            for (ii nz = ijs_[i]; nz < ijs_[i + 1]; nz++)
            {
                if (vs_[nz] > threshold)
                {
                    ...
                    for (ii k = 0; k < ii(shared.size()); k++)
                        shared[k].vs_[a_nz] = children_[k]->vs_[nz];
        ...
        a.commitCsr(isSorted_);
        swap(a);

        for (ii k = 0; k < ii(shared.size()); k++)
        {
            shared[k].commitCsr(isSorted_);
            children_[k] = a.children_[k];
            children_[k]->swap(shared[k]);
        }
    }
...
void MatrixSparse::divNonzeros(MatrixSparse& a, MatrixSparse& b)
{
    if (getDebugLevel() % 10 >= 4)
    {
        ostringstream oss;
    ...
    if (this != &a && this != &b)
        createCsrWithSharedIndexes(a);

    if (initCsr(nnz() > 0))
    {
        assert(a.ijs_ == b.ijs_);
...
ii MatrixSparse::pruneRows(const MatrixSparse &a, const MatrixSparse &b, bool bRows, fp threshold)
{
    ...
    if (a.nnz() > 0)
    {
        ...
        if (b.nnz() > 0)
        {
            ...
            {
                for (ii i = 0; i < b.m_; i++)
                    rowOrColNnzs[i] = b.ijs_[i + 1] - b.ijs_[i];
            }
            else
            {
            ...
            if (bNnzRowsOrCols / fp(aNnzRows) < threshold)
            {
                ...
                ijs_[0] = 0;
                for (ii i = 0; i < m_; i++)
                    ijs_[i + 1] = ijs_[i] + (rowOrColNnzs[i] > 0 ? a.ijs_[i + 1] - a.ijs_[i] : 0);

                //#pragma omp parallel
                for (ii i = 0; i < m_; i++)
                {
                    if (ijs_[i + 1] - ijs_[i] > 0)
                    {
                        IppStatus status = ippsCopy_32s(&a.js_[a.ijs_[i]], &js_[ijs_[i]], ijs_[i + 1] - ijs_[i]);
                        assert(!status);
                    }
                }
                ...
                for (ii i = 0; i < m_; i++)
                {
                    if (ijs_[i + 1] - ijs_[i] > 0)
                    {
                        IppStatus status = ippsCopy_32f(&a.vs_[a.ijs_[i]], &vs_[ijs_[i]], ijs_[i + 1] - ijs_[i]);
                        assert(!status);
                    }
                }
...
void MatrixSparse::exportToCoo(ii *is, ii *js, fp *vs) const
{
    ...
    if (initCsr(nnz() > 0))
    {
        ii job[] = { 0, 0, 0, 0 , nnz_, 3 };
        ii info;
        ii nnz = nnz_;
        mkl_scsrcoo(job, &m_, vs_, js_, ijs_, &nnz, vs, is, js, &info);
    }
...
void MatrixSparse::mul(fp beta)
{
    ...
    {
        IppStatus status;
        status = ippsMulC_32f_I(beta, vs_, nnz_);
        assert(!status);
...
void MatrixSparse::pow(const MatrixSparse& a, fp power)
{
    ...
    if (this != &a && createCsr(a.m(), a.n(), a.nnz()))
    {
        ...
        status = ippsCopy_32s(a.js_, js_, nnz_);
        assert(!status);
    }
...
bool MatrixSparse::createCsr(ii m, ii n, ii nnz)
{
    ...
    if (nnz > 0)
    {
        ...
        isCsrIndexesOwned_ = true;

        vs_ = static_cast<fp*>(mkl_malloc(sizeof(fp) * nnz, 64));
        isCsrValuesOwned_ = true;

        // todo: defer this MKL setup to initMkl
        ii* ijs1 = ijs_ + 1;
        sparse_status_t status = mkl_sparse_s_create_csr(&mat_, SPARSE_INDEX_BASE_ZERO, m_, n_, ijs_, ijs1, js_, vs_);
        assert(!status);
...
bool MatrixSparse::createCsrWithSharedIndexes(MatrixSparse& x)
{
    init(x.m_, x.n_);
    ...
    if (x.nnz_ > 0)
    {
        ...
        isCsrIndexesOwned_ = false;
        parent_ = &x;
        x.children_.push_back(this);

        vs_ = static_cast<fp*>(mkl_malloc(sizeof(fp) * nnz_, 64));
        isCsrValuesOwned_ = true;
...
void MatrixSparse::commitMkl(bool isSorted) const
{
    ...
    ii* ijs1;
    status = mkl_sparse_s_export_csr(mat_, &indexing, &_m_, &_n_, &_ijs_, &ijs1, &_js_, &_vs_);
    assert(!status);
    assert(!indexing);

    ii& _nnz_  = const_cast<ii&>(nnz_);
    _nnz_ = ijs_[m_];
}
...
void MatrixSparse::sort() const
{
    if (!isSorted_)
    {
        if (initCsr(nnz() > 0))
        {
            ...
            for (ii i = 0; i < m_; i++)
            {
                for (ii nz = ijs_[i]; nz < ijs_[i + 1] - 1; nz++)
                {
                    if (js_[nz] > js_[nz + 1])
                    {
                        isSorted = false;
            ...
            if (!isSorted)
            {
                ...
                double sortStart = getElapsedTime();
                {
                    ...
                    for (ii i = 0; i < m_; i++)
                    {
                        ...
                        {
                            ii bufSize;
                            IppStatus status = ippsSortRadixIndexGetBufferSize(ijs_[i + 1] - ijs_[i], ipp32s, &bufSize);
                            assert(!status);

                            Ipp8u* buffer = static_cast<Ipp8u*>(mkl_malloc(sizeof(Ipp8u) * bufSize, 64));

                            ii* idxs = static_cast<ii*>(mkl_malloc(sizeof(ii) * (ijs_[i + 1] - ijs_[i]), 64));
                            status = ippsSortRadixIndexAscend_32s(&js_[ijs_[i]], sizeof(ii), idxs, ijs_[i + 1] - ijs_[i], buffer);
                            assert(!status);
                            mkl_free(buffer);

                            ii* newJs = static_cast<ii*>(mkl_malloc(sizeof(fp) * (ijs_[i + 1] - ijs_[i]), 64));
                            for (ii nz = 0; nz < ijs_[i + 1] - ijs_[i]; nz++)
                                newJs[nz] = js_[ijs_[i] + idxs[nz]];

                            fp* newVs = static_cast<fp*>(mkl_malloc(sizeof(fp) * (ijs_[i + 1] - ijs_[i]), 64));
                            vsPackV(ijs_[i + 1] - ijs_[i], &vs_[ijs_[i]], idxs, newVs);
                            int err = vmlGetErrStatus();
                            assert(err == VML_STATUS_OK);
                            mkl_free(idxs);

                            status = ippsCopy_32s(newJs, &js_[ijs_[i]], ijs_[i + 1] - ijs_[i]);
                            assert(!status);
                            mkl_free(newJs);

                            status = ippsCopy_32f(newVs, &vs_[ijs_[i]], ijs_[i + 1] - ijs_[i]);
                            assert(!status);
                            mkl_free(newVs);
                        }
                    }
...
MatrixSparseView::MatrixSparseView(const MatrixSparse &a, ii row) : isOwned_(false)
{
    ...
    if (initCsr(a.nnz() > 0))
    {
        ii newNnz = a.ijs_[row + 1] - a.ijs_[row];
        ...
        if (newNnz > 0)
        {
            ...
            sparse_status_t status;
            ii* ijs1 = ijs_ + 1;
            status = mkl_sparse_s_create_csr(&mat_, SPARSE_INDEX_BASE_ZERO, m_, n_, ijs_, ijs1, js_, vs_);
            assert(!status);
