...
int main(int argc, char **argv)
{
    ...
    if(process == CENT1DIM || process == CENT2DIM)
    {
        ...
        mzMLb3DF.readVector(mzMLbuff, "mzML", 0);
    ...
    double mzRes= smoDF.searchGroup<double>(2);
    cout<<"MZ_resolution: "<<mzRes<<endl;
    double rtRes= smoDF.searchGroup<double>(3);
    cout<<"RT resolution: "<<rtRes<<endl;

    exit(0); // fix: smoDF.readAttribute(offset, "Offset", dataSetList[0].varid);
    smoDF.readExtent(dataMatLen, dataSetList[0].varName, dataSetList[0].grpid);
    ...
    if(process == CENT1DIM)
    {
        ...
        dims[0]=uli(rdLen[0]);
        dims[1]=uli(rdLen[1]);
        hypIdx[1]=0; // = Always readMatrixSparse from first Column;
    ...
    if(process == CENT1DIM || process == CENT2DIM)
    {
        ...
        vector<size_t> rdims;
        mzMLb3DF.readExtent(rdims, "spectrum_MS_1000514");
        ...
        mzMLb3NCDF4.writeVector(mzMLbuff, "mzML");
        vector<unsigned int> mzMLver;
        mzMLver.push_back(3);
        mzMLb3NCDF4.writeAttribute(mzMLver, "mzML", "mzMLb_version");

        if(mzMLb3DF.exists("chromatogram_MS_1000595"))
        {
            findVecString(mzMLbuff, chromatSpecIdx,"<chromatogram index","</chromatogram>");
            mzMLb3DF.readVector(chromatTime, "chromatogram_MS_1000595", 0);
            mzMLb3DF.readVector(chromatInten, "chromatogram_MS_1000515", 0);
            mzMLb3NCDF4.writeVector(chromatTime, "chromatogram_MS_1000595");
            mzMLb3NCDF4.writeVector(chromatInten, "chromatogram_MS_1000515");
        }
        mzMLb3NCDF4.writeVector(chromatSpecIdx, "mzML_chromatogramIndex");

        mzMLb3NCDF4.writeVector(mzpkSpecIdx, "mzML_spectrumIndex");
        // Unlimited dimensions
        mzMLb3NCDF4.writeVector(specMZAxisRow, "spectrum_MS_1000514_row");
        mzMLb3NCDF4.writeVector(specPKAxisRow, "spectrum_MS_1000515_row");
        mzMLb3NCDF4.writeVector(specMZAxisCol, "spectrum_MS_1000514_col");
        mzMLb3NCDF4.writeVector(specPKAxisCol, "spectrum_MS_1000515_col");
