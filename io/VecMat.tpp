...
template<typename T>
VecMat<T>::VecMat(li _r, li _c, const vector<T> &_vec):v(_vec),row(_r), col(_c)
{
    matIdx.resize(row,0);
    for(li i=0; i < row; ++i)
        matIdx[i]=&v[i*col];
...
template<typename T>
VecMat<T>::VecMat(li _r, li _c) : row(_r), col(_c)
{
    v.resize(row*col);
    matIdx.resize(row,0);
    for(li i=0; i < row; ++i)
...
template<typename T>
void VecMat<T>::set(li _r, li _c, const vector<T> &_vec)
{
    this->clear();
    row=_r;
    col=_c;
...
template<typename T>
void VecMat<T>::getDims(li dims[]) const
{
    dims[0]=row;
    dims[1]=col;
}
