...
BasisBsplinePsf::~BasisBsplinePsf()
{
}
...
synthesize(vector<MatrixSparse> &f, const vector<MatrixSparse> &x, bool accumulate, bool prune)
{
    ...
    {
        ostringstream oss;
        oss << getTimeStamp() << "     " << getIndex() << " BasisBsplinePsf::synthesize";
        info(oss.str());
    }
...
void BasisBsplinePsf::analyze(vector<MatrixSparse>& xE, const vector<MatrixSparse>& fE, bool sqrA)
{
    if (getDebugLevel() % 10 >= 3)
    {
        ostringstream oss;
        oss << getTimeStamp() << "     " << getIndex() << " BasisBsplinePsf::analyse";
        info(oss.str());
    }
